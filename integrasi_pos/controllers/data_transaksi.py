...
class DataTransaksi:
    ...
    def transfer_TSOUT_NEW(self, model_name, fields, description, date_from, date_to, limit=30):
        offset = 0

        # Cache data before starting asynchronous processing
        self.cache_data()

        while True:
            # Fetch data from the source with limit and offset
            Ts_Out_data_source = self.source_client.call_odoo('object', 'execute_kw', self.source_client.db,
                                                            self.source_client.uid, self.source_client.password,
                                                            model_name, 'search_read',
                                                            [[['picking_type_id.name', '=', 'TS Out'], ['is_integrated', '=', False], ['state', '=', 'done'], ['create_date', '>=', date_from], ['create_date', '<=', date_to]]],
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future_to_record = {executor.submit(self.process_record, record): record for record in Ts_Out_data_source}
                ...
                for future in concurrent.futures.as_completed(future_to_record):
                    ...
                    try:
                        ...
                        print(f"Record {record.get('id')} generated an exception: {exc}")

            # Increase offset for the next batch
            offset += limit

    def cache_data(self):
        # Fetch and cache warehouse data
        self.warehouse_data = self.source_client.call_odoo('object', 'execute_kw', self.source_client.db,
                                                        self.source_client.uid, self.source_client.password,
                                                        'master.warehouse', 'search_read',
                                                        [[]], {'fields': ['id', 'warehouse_code', 'warehouse_name', 'warehouse_transit'], 'limit': 1000})

        # Fetch and cache product data
        self.product_data = self.source_client.call_odoo('object', 'execute_kw', self.source_client.db,
                                                        self.source_client.uid, self.source_client.password,
                                                        'product.product', 'search_read',
                                                        [[]], {'fields': ['id', 'default_code'], 'limit': 1000})

        # Fetch and cache location data
        self.location_data = self.source_client.call_odoo('object', 'execute_kw', self.source_client.db,
                                                        self.source_client.uid, self.source_client.password,
                                                        'stock.location', 'search_read',
    ...
    def process_record(self, record):
        try:
            ...
            if target_location:
                ...
                target_location_source = next((item for item in warehouse_data if item['id'] == target_location), None)
                if target_location_source:
                    warehouse_code = target_location_source.get('warehouse_code')
                else:
                    print(f"Tidak dapat menemukan 'warehouse_code' untuk target_location {target_location} di database sumber.")
                    return
            else:
            ...
            if location_id:
                ...
                location_source = next((item for item in location_data if item['id'] == location_id), None)
                if location_source:
                    complete_name = location_source.get('complete_name')
                else:
                    print(f"Tidak dapat menemukan 'complete_name' untuk location_id {location_id} di database sumber.")
                    return
            else:
            ...
            location_target = next((item for item in location_data if item['complete_name'] == complete_name), None)
            if location_target:
                location_target_id = location_target.get('id')
            else:
                print(f"Tidak dapat menemukan 'complete_name' {complete_name} di database target.")
                return
            ...
            if target_destination_id:
                ...
                target_dest_source = next((item for item in warehouse_data if item['id'] == target_destination_id), None)
                if target_dest_source:
                    warehouse_transit_name = target_dest_source.get('warehouse_transit')
                    warehouse_target = next((item for item in location_data if item['complete_name'] == warehouse_transit_name), None)
                    if warehouse_target:
                        transit_dest = warehouse_target.get('id')
                    else:
                        print(f"Tidak dapat menemukan 'location_transit' {warehouse_transit_name} di database target.")
                        return
                else:
                    print(f"Tidak dapat menemukan 'warehouse_name' untuk target_destination_id {target_destination_id} di database sumber.")
                    return
            else:
                print(f"Transaksi dengan ID {record.get('id')} tidak memiliki 'target_destination_id'.")
                return
            ...
            for line in tsout_transfer_inventory_lines:
                ...
                if product_id:
                    ...
                    product_source = next((item for item in product_data if item['id'] == product_id), None)
                    if product_source:
                        default_code = product_source.get('default_code')
                        product_target = next((item for item in product_data if item['default_code'] == default_code), None)
                        if product_target:
                            product_id = product_target.get('id')
                        else:
                            print(f"Tidak dapat menemukan product dengan 'default_code' {default_code} di database target.")
                            missing_products.append(default_code)
                            continue
                ...
                tsin_transfer_inventory_line_data = {
                    ...
                    'name': line.get('name'),
                    'quantity': line.get('quantity'),
                    'location_dest_id': location_target_id,  # Use the same target location as TS Out
            ...
            if missing_products:
                missing_products_str = ", ".join(missing_products)
                print(f"Product berikut tidak ditemukan di target: {missing_products_str}")
                return

            # Create TS Out and TS In
            internal_transfer_data = {
                'picking_type_id': self.get_internal_transfer_picking_type_id(),
                'location_id': location_target_id,
                'location_dest_id': transit_dest,
                'move_lines': tsout_transfer_inventory_line_ids,
            ...
            new_pos_order_id = self.target_client.call_odoo('object', 'execute_kw', self.target_client.db,
                                                            self.target_client.uid, self.target_client.password,
                                                            'stock.picking', 'create', [internal_transfer_data])

            print(f"TS Out baru telah dibuat di target dengan ID: {new_pos_order_id}")

            # Post the new TS Out
            self.target_client.call_odoo('object', 'execute_kw', self.target_client.db,
                                        self.target_client.uid, self.target_client.password,
                                        'stock.picking', 'button_validate', [new_pos_order_id])

            print(f"TS Out dengan ID: {new_pos_order_id} telah diposting.")

            # Update 'is_integrated' status in source
            self.source_client.call_odoo('object', 'execute_kw', self.source_client.db,
                                        self.source_client.uid, self.source_client.password,
                                        'stock.picking', 'write', [[record['id']], {'is_integrated': True}])

            print(f"Status 'is_integrated' untuk TS Out ID: {record.get('id')} telah diperbarui.")

            # Create TS In
            ts_in_transfer_data = {
                'picking_type_id': self.get_internal_transfer_picking_type_id_tsin(),
                'location_id': transit_dest,
                'location_dest_id': location_target_id,
                'move_lines': tsin_transfer_inventory_line_ids,
                'origin': record.get('name'),
                'is_integrated': True,
                'vit_trxid': record.get('name')
            }

            new_tsin_id = self.target_client.call_odoo('object', 'execute_kw', self.target_client.db,
                                                    self.target_client.uid, self.target_client.password,
                                                    'stock.picking', 'create', [ts_in_transfer_data])

            print(f"TS In baru telah dibuat di target dengan ID: {new_tsin_id}")

            # Post the new TS In
            self.target_client.call_odoo('object', 'execute_kw', self.target_client.db,
                                        self.target_client.uid, self.target_client.password,
                                        'stock.picking', 'button_validate', [new_tsin_id])

            print(f"TS In dengan ID: {new_tsin_id} telah diposting.")

        except Exception as e:
            print(f"Gagal memproses transaksi dengan ID {record.get('id')}: {e}")

    def get_internal_transfer_picking_type_id(self):
        # Fetch the internal transfer picking type ID from source
        picking_type = self.source_client.call_odoo('object', 'execute_kw', self.source_client.db,
                                                self.source_client.uid, self.source_client.password,
                                                'stock.picking.type', 'search_read',
                                                [[['name', '=', 'Internal Transfer']]], {'fields': ['id']})
        return picking_type[0]['id'] if picking_type else None

    def get_internal_transfer_picking_type_id_tsin(self):
        # Fetch the TS In picking type ID from source
        picking_type = self.source_client.call_odoo('object', 'execute_kw', self.source_client.db,
                                                self.source_client.uid, self.source_client.password,
                                                'stock.picking.type', 'search_read',
                                                [[['name', '=', 'TS In']]], {'fields': ['id']})
        return picking_type[0]['id'] if picking_type else None
    ...
    def transfer_goods_receipt(self, model_name, fields, description, limit=100):
        max_retries = 3  # Jumlah maksimum percobaan ulang jika terjadi error
        retry_delay = 5  # Waktu tunda (dalam detik) sebelum melakukan retry

        def fetch_data(client, model, domain, fields, offset=0, limit=100):
            retries = 0
            while retries < max_retries:
                try:
                    return client.call_odoo(
                        'object', 'execute_kw', client.db,
                        client.uid, client.password,
                        model, 'search_read',
                        [domain], {'fields': fields, 'offset': offset, 'limit': limit}
                    )
                except Exception as e:
                    retries += 1
                    print(f"Error fetching data (retry {retries}/{max_retries}): {e}")
                    time.sleep(retry_delay)
            return []

        def process_record(record, source_client, target_client):
            try:
                location_id = record.get('location_id')[0] if isinstance(record.get('location_id'), list) else record.get('location_id')
                ...
                location_source = fetch_data(source_client, 'stock.location', [['id', '=', location_id]], ['complete_name'])
                location_dest_source = fetch_data(source_client, 'stock.location', [['id', '=', location_dest_id]], ['complete_name'])

                if not location_source or 'complete_name' not in location_source[0]:
                    print(f"Tidak dapat menemukan 'complete_name' untuk location_id {location_id} di database sumber.")
                    return
                if not location_dest_source or 'complete_name' not in location_dest_source[0]:
                    print(f"Tidak dapat menemukan 'complete_name' untuk location_dest_id {location_dest_id} di database sumber.")
                    return

                complete_name = location_source[0]['complete_name']
                complete_name_dest ="Partners/Vendors"

                location_target = fetch_data(target_client, 'stock.location', [['complete_name', '=', complete_name]], ['id'])
                location_dest_target = fetch_data(target_client, 'stock.location', [['complete_name', '=', complete_name_dest]], ['id'])
                ...
                goods_receipt_inventory_lines = fetch_data(source_client, 'stock.move', [['picking_id', '=', record['id']]], ['product_id', 'product_uom_qty', 'quantity', 'name'])
                ...
                for line in goods_receipt_inventory_lines:
                    ...
                    product_source = fetch_data(source_client, 'product.product', [['id', '=', product_id]], ['default_code'])
                    ...
                    default_code = product_source[0]['default_code']
                    product_target = fetch_data(target_client, 'product.product', [['default_code', '=', default_code]], ['id'])
                    ...
                    target_product_id = product_target[0]['id']
                ...
                picking_type_id = record.get('picking_type_id')[0] if isinstance(record.get('picking_type_id'), list) else record.get('picking_type_id')
                picking_types = fetch_data(source_client, 'stock.picking.type', [['id', '=', picking_type_id]], ['name'])

                if not picking_types:
                    print(f"Tidak dapat menemukan tipe picking dengan ID '{picking_type_id}' di database sumber.")
                    return

                picking_types_name = picking_types[0]['name']
                picking_types_target = fetch_data(target_client, 'stock.picking.type', [['name', '=', picking_types_name]], ['id'])

                if not picking_types_target:
                    print(f"Tidak dapat menemukan tipe picking dengan nama '{picking_types_name}' di database target.")
                    return

                picking_type_target_id = picking_types_target[0]['id']
                ...
                internal_transfer_data = {
                    ...
                    'location_id': location_dest_target_id,
                    'location_dest_id': location_target_id,
                    'picking_type_id': picking_type_target_id,
                    'move_ids_without_package': goods_receipt_inventory_line_ids,
                }

                try:
                    new_pos_order_id = target_client.call_odoo('object', 'execute_kw', target_client.db,
                                                            target_client.uid, target_client.password,
                                                            'stock.picking', 'create', [internal_transfer_data])
                    print(f"Goods Receipt baru telah dibuat dengan ID: {new_pos_order_id}")

                    target_client.call_odoo('object', 'execute_kw', target_client.db,
                                        target_client.uid, target_client.password,
                                        'stock.picking', 'button_validate', [new_pos_order_id])
                    print(f"Goods Receipt dengan ID: {new_pos_order_id} telah diposting.")

                    start_time = time.time()
                    source_client.call_odoo('object', 'execute_kw', source_client.db,
                                            source_client.uid, source_client.password,
                                            'stock.picking', 'write', [[record['id']], {'is_integrated': True, 'vit_trxid': record['name']}])
                    end_time = time.time()
                    duration = end_time - start_time
        ...
        offset = 0
        while True:
            transaksi_goods_receipt = fetch_data(self.source_client, model_name, [['picking_type_id.name', '=', 'Goods Receipts'], ['is_integrated', '=', False], ['state', '=', 'done']], fields, offset=offset, limit=limit)

            if not transaksi_goods_receipt:
                break

            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                futures = [executor.submit(process_record, record, self.source_client, self.target_client) for record in transaksi_goods_receipt]

                for future in concurrent.futures.as_completed(futures):
                    try:
                        future.result()
                    except Exception as exc:
                        print(f'Transaksi menyebabkan error: {exc}')
