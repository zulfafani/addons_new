...
export class EndShiftSessionButtonPopup extends AbstractAwaitablePopup {
    ...
    async fetchLatestCashierLog(currentSession) {
        try {
            const cashierLogs = await this.orm.searchRead(
                ...
                { order: 'timestamp desc', limit: 1 }  // Changed to desc to get the latest log
            );
    ...
    setStartShiftField(timestamp) {
        ...
        this.state.startShiftTime = this.formatDateToLocal(date);
    ...
    setEndingShiftField() {
        if (this.endingShiftField && this.endingShiftField.el) {
            const now = new Date();
            this.endingShiftField.el.value = this.formatDateToLocal(now);
        } else {
    ...
    async confirm() {
        ...
        try {
            ...
            for (const order of posOrders) {
                ...
                for (const payment of payments) {
                    ...
                    if (methodId in paymentData) {
                        ...
                        // Update payment_date if this payment is more recent
            ...
            await this.orm.write('end.shift', [endShiftId], {
                end_date: formatDate(endingShiftField),
            ...
            if (cashierLogs.length > 0) {
                await this.orm.write('pos.cashier.log', [cashierLogs[0].id], { state: 'closed' });
            }
    
            alert(_t("Success: Shift data has been updated and closed successfully."));
            this.cancel();
        } catch (error) {
